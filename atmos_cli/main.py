...
def validate_latitude(lat: float) -> bool:
    """Validates if the given latitude is within the valid range (-90 to 90)."""
    return -90 <= lat <= 90

def validate_longitude(lon: float) -> bool:
    """Validates if the given longitude is within the valid range (-180 to 180)."""
    return -180 <= lon <= 180
...
def forecast(
    ...
    Example: atmos forecast --location "London"
    Example: atmos forecast --latitude 34.05 --longitude -118.25 --hourly temperature_2m
    """
